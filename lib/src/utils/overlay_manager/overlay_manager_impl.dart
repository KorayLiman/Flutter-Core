...
abstract interface class IFLOverlayManager {
  ...
  bool isOverlayOpen({required String id});
}

/// Advanced [FLOverlayManager] for showing toasts or custom overlays.
@immutable
class FLOverlayManager implements IFLOverlayManager {
  FLOverlayManager({required GlobalKey<NavigatorState> navigatorKey}) : _key = navigatorKey;
  ...
  OverlayState get _overlayState {
    ...
    assert(ovState != null, 'Tried to show toast but overlayState was null. Key was :$_key');
    return ovState!;
  }
  ...
  void _presentToast({
    ...
    if (child != null && (leading != null || title != null || message != null)) {
  ...
  /// Checks if an overlay with [id] is open
  @override
  bool isOverlayOpen({required String id}) {
    return _overlayEntryInfoList.any((element) => element.id == id);
  }
}

/// Error thrown when [FLOverlayManager] is misused
